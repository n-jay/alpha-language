...
#define Y_verify(i) Y_verify[i]
#define var_Y(i) Y(i)
#define var_Y_verify(i) Y_verify(i)

//function prototypes
void PrefixScan(long, float*, float*);
void PrefixScan_verify(long, float*, float*);
...
int main(int argc, char** argv) {
	...
	#ifdef VERIFY
		float* Y_verify = (float*)malloc(sizeof(float)*(N));
		mallocCheck(Y_verify, (N), float);
	#endif
	...
	//Verification Run
			...
			elapsed_time = (((double) time.tv_sec) + ((double) time.tv_usec)/1000000);
		#endif
    	PrefixScan_verify(N, X, Y_verify);
    	#ifdef TIMING
    		gettimeofday(&time, NULL);
			elapsed_time = (((double) time.tv_sec) + ((double) time.tv_usec)/1000000) - elapsed_time;
		...
		
		{
			...
			#else
				#define S0(i) printf("Y(%ld)=",(long) i);printf("%0.2f\n",var_Y(i))
			#endif
			int c1;
			for(c1=0;c1 <= N-1;c1+=1)
			 {
		...
		//Compare outputs for verification
		{
			//Error Counter
			int _errors_ = 0;
			#define S0(i) if (fabsf(1.0f - var_Y_verify(i)/var_Y(i)) > EPSILON) _errors_++;
			int c1;
			for(c1=0;c1 <= N-1;c1+=1)
			 {
			 	S0((c1));
