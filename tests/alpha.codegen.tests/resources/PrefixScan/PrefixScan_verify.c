...
float reduce_PrefixScan_verify_Y_1(long, int);
...
void PrefixScan_verify(long N, float* _local_X, float* _local_Y){
	///Parameter checking
	if (!((N >= 1))) {
...
float reduce_PrefixScan_verify_Y_1(long N, int ip){
	float reduceVar = 0;
	#define S0(i,j) reduceVar = (reduceVar)+(X(j))
	{
		//Domain
...
float eval_verify_Y(long N, int i){
	if ( _flag_Y(i) == 'N' ) {
		_flag_Y(i) = 'I';
	//Body for Y
		Y(i) = reduce_PrefixScan_verify_Y_1(N,i);
		_flag_Y(i) = 'F';
	} else if ( _flag_Y(i) == 'I' ) {
