/**
 * generated by Xtext 2.12.0
 */
package alpha.alpha;

import org.eclipse.emf.common.util.EList;

import org.eclipse.emf.ecore.EObject;

/**
 * <!-- begin-user-doc -->
 * A representation of the model object '<em><b>AIndex List</b></em>'.
 * <!-- end-user-doc -->
 *
 * <p>
 * The following features are supported:
 * </p>
 * <ul>
 *   <li>{@link alpha.alpha.AIndexList#getIds <em>Ids</em>}</li>
 * </ul>
 *
 * @see alpha.alpha.AlphaPackage#getAIndexList()
 * @model
 * @generated
 */
public interface AIndexList extends EObject
{
  /**
   * Returns the value of the '<em><b>Ids</b></em>' containment reference list.
   * The list contents are of type {@link alpha.alpha.AIndexDimension}.
   * <!-- begin-user-doc -->
   * <p>
   * If the meaning of the '<em>Ids</em>' containment reference list isn't clear,
   * there really should be more of a description here...
   * </p>
   * <!-- end-user-doc -->
   * @return the value of the '<em>Ids</em>' containment reference list.
   * @see alpha.alpha.AlphaPackage#getAIndexList_Ids()
   * @model containment="true"
   * @generated
   */
  EList<AIndexDimension> getIds();

} // AIndexList
