...
float reduce_fsub_abft_verify_x_c_0_1(long);
float eval_verify_x_c_0(long);
float reduce_fsub_abft_verify_x_c_1_2(long, int);
float reduce_fsub_abft_verify_x_c_1_1(long);
...
void fsub_abft_verify(long N, float** _local_L, float* _local_b, float* _local_x, float* _local_Inv_x_c, float* _local_x_c_0, float* _local_x_c_1){
	///Parameter checking
	if (!((N >= 1))) {
...
float reduce_fsub_abft_verify_x_c_0_1(long N){
	float reduceVar = 0;
...
float eval_verify_x_c_0(long N){
	if ( _flag_x_c_0 == 'N' ) {
		_flag_x_c_0 = 'I';
	//Body for x_c_0
		x_c_0 = reduce_fsub_abft_verify_x_c_0_1(N);
		_flag_x_c_0 = 'F';
...
float reduce_fsub_abft_verify_x_c_1_2(long N, int ip){
	float reduceVar = 0;
	#define S0(i,j) reduceVar = (reduceVar)+((L(i,j))*(eval_verify_x(N,j)))
	{
		//Domain
...
float reduce_fsub_abft_verify_x_c_1_1(long N){
	float reduceVar = 0;
	#define S0(i) reduceVar = (reduceVar)+((((i == 0))?(b(i))/(L(i,i)):(((b(i))-(reduce_fsub_abft_verify_x_c_1_2(N,i)))/(L(i,i)))))
	{
		//Domain
		//{i|i>=0 && N>=i+1 && N>=1}
		int c1;
...
float eval_verify_x_c_1(long N){
	if ( _flag_x_c_1 == 'N' ) {
		_flag_x_c_1 = 'I';
	//Body for x_c_1
		x_c_1 = reduce_fsub_abft_verify_x_c_1_1(N);
		_flag_x_c_1 = 'F';
