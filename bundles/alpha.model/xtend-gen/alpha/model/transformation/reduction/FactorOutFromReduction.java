...
public class FactorOutFromReduction {
  ...
  private BinaryExpression transform() {
    ...
    final ISLMultiAff projectedDep = AffineFunctionOperations.projectFunctionDomain(this.targetExpr.getFunction(), this.targetReduce.getProjection());
    final DependenceExpression newDepExpr = AlphaUserFactory.createDependenceExpression(projectedDep, this.targetExpr.getExpr());
    final ISLSet projectedContext = this.targetExpr.getContextDomain().apply(this.targetReduce.getProjection().toMap());
